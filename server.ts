...
// Intercept console.error and console.warn to suppress harmless Firebase SDK BloomFilter & Baileys Bad MAC decryption warnings/errors
const originalConsoleError = console.error;
console.error = function (...args: any[]) {
  const errString = args.map(arg => typeof arg === 'object' ? JSON.stringify(arg) : String(arg)).join(' ');
  if (
    errString.includes('BloomFilter') || 
    errString.includes('Invalid hash count') ||
    errString.includes('Bad MAC') ||
    errString.includes('Failed to decrypt message') ||
    errString.includes('Session error')
  ) {
    // Gracefully ignore harmless internal logs / E2EE decryption retry events
    return;
  }
...
console.warn = function (...args: any[]) {
  ...
  if (
    warnString.includes('BloomFilter') || 
    warnString.includes('Invalid hash count') ||
    warnString.includes('Bad MAC') ||
    warnString.includes('Failed to decrypt message') ||
    warnString.includes('Session error')
  ) {
    // Gracefully ignore harmless internal logs / E2EE decryption retry events
    return;
  }
...
process.on('uncaughtException', (err: any) => {
  ...
  if (msg.includes('Bad MAC') || msg.includes('Failed to decrypt') || msg.includes('Session error') || msg.includes('Stream Errored') || msg.includes('restart required')) {
    console.log('[Baileys Signal] Handled Bad MAC / Stream error gracefully.');
    return;
  }
...
process.on('unhandledRejection', (reason: any) => {
  ...
  if (msg.includes('Bad MAC') || msg.includes('Failed to decrypt') || msg.includes('Session error') || msg.includes('Stream Errored') || msg.includes('restart required')) {
    console.log('[Baileys Signal] Handled Bad MAC / Stream error rejection gracefully.');
    return;
  }
...
// 24/7 Keep-Alive Engine state tracking
let lastSelfPingTimestamp: string | null = null;
let selfPingCount = 0;
let selfPingStatus = 'initializing';

// Start 24/7 Keep-Alive Engine to keep Render, Cloud Run, and Free-tier web services awake 24/7
function start24SevenKeepAliveEngine() {
  console.log('[Keep-Alive Engine] Starting 24/7 background heartbeat & auto-restoration worker...');

  const executePing = async () => {
    selfPingCount++;
    lastSelfPingTimestamp = new Date().toISOString();

    // Determine target URL for self ping (APP_URL or external cron ping)
    const publicUrl = process.env.APP_URL;
    
    // Ping local server endpoint
    try {
      await fetch('http://127.0.0.1:3000/api/health').catch(() => {});
    } catch (e) {}

    // If public URL exists, ping public URL to reset Render 15-minute inactivity timer
    if (publicUrl) {
      try {
        const targetEndpoint = `${publicUrl.replace(/\/$/, '')}/api/health`;
        const res = await fetch(targetEndpoint, { 
          method: 'GET', 
          headers: { 'User-Agent': 'HijjazeBot-247-KeepAlive/1.0' } 
        });
        if (res.ok) {
          selfPingStatus = `active (Ping #${selfPingCount} OK to ${targetEndpoint})`;
          console.log(`[Keep-Alive 24/7] Heartbeat ping #${selfPingCount} successful -> ${targetEndpoint}`);
        } else {
          selfPingStatus = `warning (HTTP ${res.status})`;
        }
      } catch (err: any) {
        selfPingStatus = `error (${err?.message || err})`;
        console.warn(`[Keep-Alive 24/7] Self-ping to public URL failed:`, err?.message || err);
      }
    } else {
      selfPingStatus = `active local only (Ping #${selfPingCount})`;
      console.log(`[Keep-Alive 24/7] Local heartbeat ping #${selfPingCount} completed.`);
    }
  };

  // Run initial keep-alive ping 10 seconds after boot
  setTimeout(executePing, 10000);

  // 1. Every 3 minutes (180 seconds), send keep-alive HTTP ping (Render sleep threshold is 15 mins)
  const PING_INTERVAL_MS = 3 * 60 * 1000;
  setInterval(executePing, PING_INTERVAL_MS);

  // 2. Every 5 minutes (300 seconds), verify WhatsApp sessions health and auto-reconnect any disconnected session
  const SESSION_HEALTH_CHECK_INTERVAL_MS = 5 * 60 * 1000;
  setInterval(async () => {
    console.log('[24/7 Health Check] Running periodic WhatsApp session health scan & auto-reconnect worker...');
    try {
      await autoConnectAllSessions();
    } catch (err) {
      console.error('[24/7 Health Check] Session scan error:', err);
    }
...
app.get(['/api/health', '/health', '/healthz'], (req, res) => {
  const activeSocketsMap = getAllActiveSockets();
  const activeUserIds = Array.from(activeSocketsMap.keys());

  res.json({
    status: 'ok',
    service: 'Hijjaze Bot WhatsApp Engine (24/7 Enabled)',
    ...
    keepAlive: {
      ...
      totalPings: selfPingCount,
      lastPing: lastSelfPingTimestamp,
      renderExternalUrl: process.env.RENDER_EXTERNAL_URL || null,
      configuredAppUrl: process.env.APP_URL || null,
      selfPingUrl: process.env.SELF_PING_URL || null
    },
    whatsApp: {
      activeSessionsCount: activeSocketsMap.size,
      activeUserIds: activeUserIds
    },
    deploymentNotice: 'To run 24/7 forever on Render free tier without opening Chrome, render.com automatically injects RENDER_EXTERNAL_URL. The built-in Keep-Alive engine will ping this service every 4 minutes to prevent sleep mode. You can also add https://<your-render-app>.onrender.com/api/health to UptimeRobot or Cron-Job.org for free external pings.'
  });
});
...
app.get('/api/user/profile', authenticateToken, async (req: any, res) => {
  try {
    ...
    // Check session status across sessionId, req.user.id, and email match
    ...
    const hasCreds = (await hasSavedSession(sessionId)) || (await hasSavedSession(req.user.id));
    ...
    if (hasCreds) {
      if (!session) {
        ...
        if (!session.phone && profile.whatsappPhone) {
    ...
    if (session?.phone) {
      profile.whatsappPhone = session.phone;
      updateUserProfileInfo(sessionId, { whatsappPhone: session.phone });
      if (req.user.id !== sessionId) {
        updateUserProfileInfo(req.user.id, { whatsappPhone: session.phone });
      }
    }
...
app.get('/api/whatsapp/status', authenticateToken, async (req: any, res) => {
  try {
    ...
    const profile = getUserProfile(sessionId, req.user.email, req.user.name);
    const activeSock = getActiveSocket(sessionId) || getActiveSocket(baseUserId);
    const hasCreds = (await hasSavedSession(sessionId)) || (await hasSavedSession(baseUserId)) || (activeSock !== undefined);
    ...
    if (hasCreds) {
      ...
      if (!activeSock) {
        initWhatsAppSession(sessionId, req.user.email, false, undefined, true).catch(err => {
          console.warn('[Status API] Auto-reconnect background trigger:', err?.message || err);
        });
      }
    }
...
// Send Custom WhatsApp Messages (Admin Restricted)
app.post('/api/whatsapp/send', authenticateToken, async (req: any, res) => {
  try {
    // CRITICAL: Only Admin accounts can control linked devices to send messages/pictures/videos
    if (req.user.role !== 'admin') {
      return res.status(403).json({ 
        error: 'Access denied. Only administrators have permission to control linked WhatsApp devices and send messages or media.' 
      });
    }

    const { targetPhone, messageType, content, fileName, targetSessionId, broadcastAll } = req.body;
    if (!targetPhone || !messageType || !content) {
      return res.status(400).json({ error: 'targetPhone, messageType, and content are required' });
    }

    const sessionId = targetSessionId || getSessionId(req);
    const isBroadcast = broadcastAll === true || targetSessionId === 'all';
    ...
    addLog(
      ...
      req.user.email, 
      'admin_send_message', 
      `Admin sent ${messageType} message to +${targetPhone}${isBroadcast ? ' (Broadcast across all connected devices)' : ''}`
    );
...
async function startServer() {
  ...
  // Launch 24/7 keep-alive background worker
  start24SevenKeepAliveEngine();
