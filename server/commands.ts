...
import { openAiCommands } from './openai_commands';
import { islamicCommands } from './islamic_commands';
import { validatePlatformUrl, downloadSocialMedia, fetchMediaBuffer, buildDownloaderCaption, extractUrlFromArgs } from './social_downloader';

// Helper handler for all social media downloader commands
async function handleSocialDownload(
  ctx: CommandContext,
  platformCmd: string,
  platformName: string,
  usageExample: string
  ...
  const validation = validatePlatformUrl(rawUrl, platformCmd);

  if (!rawUrl || !validation.valid) {
    const invalidBox = `❌ Invalid URL.

Usage:
.${platformCmd} <url>`;
    await sock.sendMessage(chatJid, { text: invalidBox }, { quoted: msg });
    return;
  }

  // Display status "Processing..." message
  const statusText = `🔎 Processing your request...\nPlease wait...`;
  let statusMsg: any = null;
  try {
    statusMsg = await sock.sendMessage(chatJid, { text: statusText }, { quoted: msg });
  } catch (e) {
    console.warn(`[.${platformCmd}] Could not send status message:`, e);
  }

  try {
    const result = await downloadSocialMedia(validation.cleanUrl, platformName);

    if (!result.success || !result.medias || result.medias.length === 0) {
      const errorBox = `❌ Unable to download this content.

Possible reasons:
• The post is private.
• The link is invalid.
• The content has been removed.
• The platform does not allow access to this media.`;

      if (statusMsg?.key) {
        await sock.sendMessage(chatJid, { text: errorBox, edit: statusMsg.key });
      } else {
        await sock.sendMessage(chatJid, { text: errorBox }, { quoted: msg });
      }
      return;
    }
    ...
    for (let i = 0; i < result.medias.length; i++) {
      ...
      const caption = buildDownloaderCaption(platformName, item.type, item.quality || 'Best Available');

      const fetched = await fetchMediaBuffer(item.url);
      if (!fetched || !fetched.buffer || fetched.buffer.length === 0) continue;

      if (item.type === 'image' || fetched.mimetype.startsWith('image/')) {
        ...
        sentCount++;
      } else if (item.type === 'video' || fetched.mimetype.startsWith('video/')) {
        await sock.sendMessage(chatJid, {
          video: fetched.buffer,
          caption: caption,
          mimetype: fetched.mimetype
        }, { quoted: i === 0 ? msg : undefined });
        sentCount++;
      } else if (item.type === 'audio' || fetched.mimetype.startsWith('audio/')) {
        await sock.sendMessage(chatJid, {
    ...
    if (sentCount > 0) {
      ...
      if (statusMsg?.key) {
        try {
          await sock.sendMessage(chatJid, { delete: statusMsg.key });
        } catch (e) {
          // Ignore delete error
        }
      }
    } else {
      const failBox = `⚠ Download failed.

Please try again later.`;
      if (statusMsg?.key) {
        await sock.sendMessage(chatJid, { text: failBox, edit: statusMsg.key });
      } else {
        await sock.sendMessage(chatJid, { text: failBox }, { quoted: msg });
      }
    }
  } catch (err: any) {
    console.error(`[Social Downloader Error] .${platformCmd} error:`, err);
    const failBox = `⚠ Download failed.
...
🤖 *Downloaded by Hijjaze Bot*`;
  ...
  {
    name: 'insta',
    aliases: ['ig', 'instagram', 'instadl'],
    category: '🎵 DOWNLOAD',
    description: 'Download Instagram Reels, Posts, Videos, Photos, and Carousels',
    usage: '.insta <url>',
    handler: async (ctx) => {
      await handleSocialDownload(ctx, 'insta', 'Instagram', '.insta https://www.instagram.com/reel/XXXXXXXX/');
    }
  },
  {
    name: 'fb',
    ...
    category: '🎵 DOWNLOAD',
    description: 'Download Facebook Videos, Reels, and Public Posts',
    usage: '.fb <url>',
    handler: async (ctx) => {
      await handleSocialDownload(ctx, 'fb', 'Facebook', '.fb https://www.facebook.com/...');
    }
  },
  {
    name: 'tt',
    aliases: ['tiktok', 'ttdl'],
    category: '🎵 DOWNLOAD',
    description: 'Download TikTok Videos in HD quality without watermark',
    usage: '.tt <url>',
    handler: async (ctx) => {
      await handleSocialDownload(ctx, 'tt', 'TikTok', '.tt https://www.tiktok.com/...');
    }
  },
  {
    name: 'x',
    aliases: ['twitter', 'xdl'],
    category: '🎵 DOWNLOAD',
    description: 'Download X / Twitter Videos, GIFs, and Posts',
    usage: '.x <url>',
    handler: async (ctx) => {
      await handleSocialDownload(ctx, 'x', 'X', '.x https://x.com/...');
    }
  },
  {
    name: 'pinterest',
    aliases: ['pin', 'pindl'],
    category: '🎵 DOWNLOAD',
    description: 'Download Pinterest Videos and HD Images',
    usage: '.pinterest <url>',
    handler: async (ctx) => {
      await handleSocialDownload(ctx, 'pinterest', 'Pinterest', '.pinterest https://pinterest.com/pin/...');
    }
  },
  {
    name: 'snapchat',
    aliases: ['snap', 'snapdl'],
    category: '🎵 DOWNLOAD',
    description: 'Download Snapchat Spotlight Videos and Media',
    usage: '.snapchat <url>',
    handler: async (ctx) => {
      await handleSocialDownload(ctx, 'snapchat', 'Snapchat', '.snapchat https://snapchat.com/spotlight/...');
    }
  },
  {
    name: 'threads',
    ...
    category: '🎵 DOWNLOAD',
    description: 'Download Threads Posts, Videos, and Photos',
    usage: '.threads <url>',
    handler: async (ctx) => {
      await handleSocialDownload(ctx, 'threads', 'Threads', '.threads https://www.threads.net/t/...');
    }
  },
  ...openAiCommands,
  ...islamicCommands
...
export function getDisplayCategoryTitle(cmd: Command): string {
  ...
  if (cleanCat.includes('SEARCH') || cleanCat.includes('KNOWLEDGE')) return `🌍 ${toBoldSans('KNOWLEDGE & SEARCH')}`;
...
export async function handleIncomingMessage(sock: any, msg: any, userId: string, email: string) {
  ...
  // Wrapped sock proxy to intercept and beautifully format responses and attach native newsletter context
      ...
      if (prop === 'sendMessage') {
        return async (jid: string, content: any, options: any = {}) => {
          try {
            ...
            console.error('[Proxy sendMessage] Error formatting response with newsletter context:', err);
          }
