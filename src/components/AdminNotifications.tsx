...
export default function AdminNotifications({ authToken, theme = 'dark', onNavigateTab }: AdminNotificationsProps) {
  ...
  // Fetch initial notifications with automatic retry
  const fetchNotifications = async (retryCount = 2) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.warn(`Admin notifications fetch returned status ${res.status}`);
      }
    } catch (err: any) {
      ...
      console.warn('Unable to reach admin notifications endpoint:', err?.message || err);
    }
  };
